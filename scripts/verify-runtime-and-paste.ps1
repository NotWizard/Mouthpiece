...
Write-Host "=== Mouthpiece Runtime Verification ==="
...
if (-not $procs) {
  Write-Host "[WARN] No running Mouthpiece/Electron process found."
} else {
...
$fingerprintFileCandidates = @(
  (Join-Path $env:APPDATA "Mouthpiece-development\runtime-fingerprint.json"),
  (Join-Path $env:APPDATA "Mouthpiece\runtime-fingerprint.json")
)
...
$devLogDir = Join-Path $env:APPDATA "Mouthpiece-development\logs"
$prodLogDir = Join-Path $env:APPDATA "Mouthpiece\logs"
