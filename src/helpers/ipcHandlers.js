...
class IPCHandlers {
  ...
  setupHandlers() {
    ...
    // Runtime config (apiUrl / authUrl / oauth* / cloud toggle) is built once
    // in src/helpers/runtimeConfig.js and injected into each renderer via
    // webPreferences.additionalArguments at window creation time, so preload
    // no longer needs ipcRenderer.sendSync(). Keep the async IPC fallback for
    // any out-of-band caller (debug shells, future-use).
