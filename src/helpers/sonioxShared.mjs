...
export const createInitialSonioxTranscriptState = () => ({
...
export const hasSonioxFinalizeToken = (tokens = []) =>
 ...
 */
export const selectSonioxModel = ({ requestedModel, realtimeEnabled = false } = {}) => {
  if (realtimeEnabled) {
...
export const buildSonioxRealtimeConfig = ({
  apiKey,
  model,
...
export const buildSonioxAsyncPayload = ({ model, language, keyterms = [] } = {}) => {
  const payload = {
...
export const accumulateSonioxTokens = (
  state = createInitialSonioxTranscriptState(),
  tokens = []
) => {
  const nextState = {
