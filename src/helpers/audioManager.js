...
const ENGLISH_STUTTER_RE = /\b(i|we|you|he|she|they|it|the|a|an|to|and|but)\b(?:\s+\1\b)+/gi;
...
`;
  ...
  stopInputLevelMonitoring() {
    ...
    if (this.inputLevelSource) {
      try {
        this.inputLevelSource.disconnect();
      } catch {
    ...
    if (this.inputLevelOwnsContext && this.inputLevelContext) {
      this.inputLevelContext.close().catch(() => {});
    }
  ...
  startInputLevelMonitoring({ stream, audioContext = null, sourceNode = null } = {}) {
    this.stopInputLevelMonitoring();

    try {
      const levelContext = audioContext || new AudioContext();
      const analyser = levelContext.createAnalyser();
      analyser.fftSize = 512;
      analyser.smoothingTimeConstant = 0.78;

      const meterSource = sourceNode || levelContext.createMediaStreamSource(stream);
      meterSource.connect(analyser);

      this.inputLevelContext = levelContext;
      this.inputLevelOwnsContext = !audioContext;
      this.inputLevelSource = sourceNode ? null : meterSource;
      this.inputLevelAnalyser = analyser;

      const dataArray = new Uint8Array(analyser.fftSize);
      this.inputLevelInterval = setInterval(() => {
      ...
      logger.warn(
        "Input level monitoring setup failed, skipping",
        { error: error.message },
        "audio"
      );
      this.emitAudioLevel(0);
  ...
  async startRecording() {
    try {
      ...
      this._peakRms = 0;
      this.startInputLevelMonitoring({ stream });
  ...
  applyDictionaryNormalization(text, source = "unknown") {
    ...
    const {
      ...
      replacements,
      matches,
    } = applyDictionaryCanonicalization(text, entries);
