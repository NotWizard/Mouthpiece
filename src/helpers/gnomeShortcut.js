...
const DBUS_SERVICE_NAME = "com.mouthpiece.App";
const DBUS_OBJECT_PATH = "/com/mouthpiece/App";
const DBUS_INTERFACE = "com.mouthpiece.App";

const KEYBINDING_PATH =
  "/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/mouthpiece/";
...
class GnomeShortcutManager {
  ...
  _createInterfaceClass(dbusModule, callback) {
    class MouthpieceInterface extends dbusModule.interface.Interface {
      constructor() {
    ...
    MouthpieceInterface.configureMembers({
      methods: {
        Toggle: { inSignature: "", outSignature: "" },
      },
    });

    return MouthpieceInterface;
  }
  ...
  async registerKeybinding(shortcut = "<Alt>r") {
    ...
    try {
      ...
      execFileSync(
        "gsettings",
        ["set", `${KEYBINDING_SCHEMA}:${KEYBINDING_PATH}`, "name", "Mouthpiece Toggle"],
        { stdio: "pipe" }
      );
      execFileSync(
        "gsettings",
