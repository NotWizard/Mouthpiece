...
const CLOUD_STANDARD_MAX_FILE_SIZE = 25 * 1024 * 1024; // 25 MB — standard cloud threshold
const CLOUD_MAX_FILE_SIZE = 500 * 1024 * 1024; // 500 MB — absolute cloud upload limit
...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  // BYOK: 25 MB hard max
  // Mouthpiece Cloud: 500 MB hard max, with chunked processing above 25 MB
  ...
  if (file) {
    if (useLocalWhisper) {
      ...
      fileTooLarge = file.sizeBytes > CLOUD_MAX_FILE_SIZE;
      isLargeFile = file.sizeBytes > CLOUD_STANDARD_MAX_FILE_SIZE;
    }
  }
    ...
    <div style={{ animation: "float-up 0.3s ease-out" }}>
      ...
      {byokTooLarge && (
        <div className="rounded-lg border border-primary/12 dark:border-primary/15 bg-primary/[0.03] px-3 py-2.5 mb-3">
          ...
          {!requiresAccount && cloudAuthAvailable && (
      ...
      {/* Cloud large file info (will be chunked) */}
      {isLargeFile && !fileTooLarge && isOpenWhisprCloud && (
      ...
      <div className="flex items-center gap-2 justify-center flex-wrap">
        ...
        {byokTooLarge && !requiresAccount && cloudAuthAvailable && (
          <Button
            variant="default"
            size="sm"
