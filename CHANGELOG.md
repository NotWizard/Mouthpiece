...
- API keys written to the keychain no longer ride iCloud Keychain sync or restore-to-another-device from an encrypted backup: `KeychainStore.write` set `kSecAttrAccessible = kSecAttrAccessibleAfterFirstUnlock` (which is syncable-and-migratable by default) only on the SecItemAdd path, so a subsequent SecItemUpdate never touched accessibility at all. Every write now pins accessibility to `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` through a single `KeychainStore.accessibility` seam that both the SecItemAdd insert dict and the SecItemUpdate attributesToUpdate dict pass to the Security framework, so a fresh insert lands device-only and an existing (pre-fix) item is upgraded to device-only on the next save. `testCredentialAccessibleAttributeIsDeviceOnlyOnAddAndUpdate` now writes via `KeychainStore.write` and reads `kSecAttrAccessible` back through `SecItemCopyMatching(kSecReturnAttributes: true)` after both the Add path and the Update overwrite, so a regression dropping the attribute from either dictionary would be caught in-process. Only newly-saved credentials are affected — items already on user machines keep their old accessibility until the user re-saves the corresponding API key (audit NEW-5).
