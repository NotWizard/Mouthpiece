...
- R2 structural batch from the 2026-08-07 improvement audit, all cross-reviewed (15/15):
  ...
  - Release chain: the DMG Finder-layout step fails loudly with a retry and read-back verification instead of `|| true` (E7), and each release's Sparkle appcast now merges up to five previous versions' entries so withdrawing a release leaves a working update path (E8, hosted on release assets — no gh-pages migration by design).
