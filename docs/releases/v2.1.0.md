...
这是一轮系统性代码审计后的集中修复版本：覆盖数据丢失、隐私泄漏、进程健壮性、发布链安全与无障碍打磨，共修复 29 项审计条目并新增回归测试（单元测试 116 → 159）。绝大多数用户无需任何操作即可获益；少数条目仅影响新保存的数据（见下）。
...
- API Key 写入钥匙串固定为「仅本机、解锁后可用」，不再随 iCloud 钥匙串同步或跨机备份恢复（仅影响此后新保存的凭据）。
...
- API keys are pinned to device-only, after-first-unlock keychain accessibility, so they no longer ride iCloud Keychain sync or restore to another device (affects only newly-saved credentials).
...
- Release artifact verification asserts the bundled model runtimes and the MediaRemote adapter are actually inside the `.app` with matching architecture; DMG packaging switched to a unique mountpoint and relative-path checksums.
