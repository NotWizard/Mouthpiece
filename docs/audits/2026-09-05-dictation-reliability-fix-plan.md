...
| 当前状态 | 四项修复已实施并通过自动化验收（2026-09-05） |
| 本次交付 | 修复方案及可重建的复现、验收说明；四项修复已落地（提交 88dea7a2 / 24bdc8cb / eb5d77d6 / 4635a232） |
...
- [x] 编写本方案。
- [x] 将四项复现纳入正式测试，先确认基线按预期失败。
- [x] 实施 F1，通过完成路径和输出策略回归。
- [x] 实施 F3，通过流错误、截断和原文回退回归。
- [x] 实施 F4，通过不合作任务、取消及迟到结果回归。
- [x] 实施 F2，通过自等待、资源释放和新会话保护回归。
- [x] 运行集成测试和全量 XCTest，记录通过、失败、跳过原因。
- [ ] 完成必要的本机交互验证，确认无真实输入目标被误写。
- [x] 按实际结果更新 CHANGELOG 和本方案状态。

实施验收记录（2026-09-05）：四项复现纳入 `DictationCoordinatorTests` 后按基线确认失败（F1 三断言、F2/F3/F4 各一如预期红）；按 F1→F3→F4→F2 顺序修复并各自转绿；全量 XCTest（方案 §9.1 命令）实测 **171 例执行、0 失败、1 跳过**（跳过为本地 keychain 提示会挂 SecurityAgent 的既有用例，与基线一致）；`git diff --check` 通过。本机交互验证（§9.2）未执行，待用户在新版上实操确认；执行前不得声称该项达标。
