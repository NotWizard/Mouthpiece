...
- [x] Apple Silicon Release Archive：主程序和 Whisper、Parakeet、llama.cpp 运行时构建成功，主程序目标为 `arm64-apple-macos15.0`。
- [x] Intel Release Archive：主程序和三套本地运行时构建成功，主程序目标为 `x86_64-apple-macos15.0`。
...
- [ ] 使用百炼、Deepgram、Soniox、AssemblyAI、OpenAI 兼容批量识别及自定义地址各完成一次真实录音。
- [ ] 使用 Whisper、Parakeet、Qwen ASR 和本地文字整理模型各完成一次真实录音或处理。
